...
def consolidate_references(reference_list):
    # Dictionary to hold the consolidation of reference IDs by unique text and link
    consolidated_dict = {}
    
    # Regex pattern to parse the reference ID, optional text, and link
    pattern = re.compile(r'(\d+),\s*(.*?)(https?://\S+)$')
    
    for entry in reference_list:
        match = pattern.search(entry.strip())
        if match:
            ref_id, text, link = match.groups()
            # Using text and link as the key to group reference IDs
            key = (text.strip(), link.strip())
            if key in consolidated_dict:
                consolidated_dict[key].append(ref_id)
            else:
                consolidated_dict[key] = [ref_id]
    
    # Generate the new list of references with combined IDs
    new_reference_list = []
    for (text, link), ref_ids in consolidated_dict.items():
        combined_ref_id = " ".join(ref_ids)
        new_entry = f"{combined_ref_id}, {text} {link}"
        new_reference_list.append(new_entry)
...
def parseMasterDBData(citations_metadata):
    ...
    for i, citation in enumerate(citations_metadata):
        if 'file_name' in citation:
            ...
            modified_citation_complete = citation["reference_id"] + "  " + reference["title"] + "  " + reference["link"]
            #we group all references to the same document on one line
            modified_citation_formatted = consolidate_references(modified_citation_complete)
            modified_citations.append(modified_citation_formatted)
        else:
            continue
