...
def fetch_url_from_filename(filename, list_bfa, list_cifar):
    ...
    if uuid_match := uuid_pattern.match(filename):
        ...
        print ("in fetch_url_from_filename,in the pattern matching code, case soq, creating metadata from project code + soq spreadsheet link"+ metadata)
        return metadata
    ...
    if len(result) == 1:
        ...
        print ("in fetch_url_from_filename, when theres exactly one row parsed, creating metadata from result list"+ metadata)
        return metadata
    else:
        metadata["title"] = "Failed to fetch the reference original url"
        metadata["link"] = ""
