    ...
    if len(citations) > 0:
        for text in citations:
            ...
            if text.endswith(".txt"):
                ...
                # pattern = r"\b\w+\sfrom\s(\d+\.txt)\b"
                pattern = r"(\d+\.txt)|\b\w+\sfrom\s(\d+\.txt)\b|\[.+\]\sNone\sfrom\s([a-f0-9\-]+\.txt)"
                match = re.search(pattern, text)
                if match:
                    # file_name = match.group(1)
                    
                    # Extracting the matched filename from one of the three groups
                    ...
                    print("Pattern matching fail, citaion: " + text)
            elif text.endswith(".pdf"):
                pdf_name_match = re.search(r"from (\d+_.+)\.pdf", text)
                if pdf_name_match:
                    pdf_name = pdf_name_match.group(1)
                    pdf_name = pdf_name + ".pdf"
                    post_id = pdf_name.split('_', 1)[0]
...
def assistant_process(dict_from_dataset):
    ...
    if not initial_tread:
        initial_tread = client.beta.threads.create()
        
    message = client.beta.threads.messages.create(
        thread_id=initial_tread.id,
        role="user",
    ...
    if run.status == 'completed':
        messages = client.beta.threads.messages.list(thread_id=initial_tread.id)
