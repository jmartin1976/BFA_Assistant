    ...
    if len(result) > 1:
        ...
        if case in ["bfaPost", "cifarPost", "cifarPdf", "bfaPdf"]:
            if case in ["cifarPdf", "bfaPdf"]:
                ...
                # Check if the link is empty, if so, attempt to get it using other keys
                if metadata["link"] == "":
                    metadata["link"] = item.get('guid', item.get('URL', ''))
                metadata["title"] = ""
            else:
                metadata["link"] = item.get('pdf_url', item.get('guid', item.get('URL', '')))
                if metadata["link"] == "":
                    metadata["link"] = item.get('guid', item.get('URL', ''))
                # When its a pdf its tagging it as post for some reason, Im deleting all titles for now, this code needs revision if titles are needed for posts
...
def parseMasterDBData(citations_metadata):
    ...
    """
    modified_citations = []
    BFA_master_db_file_path = 'data/BFA_final_master_db.csv'
    CifarWIX_master_db_file_path = 'data/CifarWIX_final_master_db.csv'
        ...
        for text in citations:
            ...
            if text.endswith(".txt"):
                ...
                if pdf_name_match:
                    ...
                    pdf_name_full = pdf_name_full + ".pdf"
                    post_id = pdf_name_full.split('_', 1)[0]
                    pdf_name = pdf_name_full.split('_', 1)[1] + ".pdf"
                    file_type = "pdf"
                    citationsAsLinks.append({
                        "reference_id": ref_id,
                        "post_id": post_id,
                        "file_name": pdf_name_full,
                        "file_type": file_type
                    })
